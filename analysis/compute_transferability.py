...
def compute_table(dataset, explainer, metric, epsilon):
    filename_label_agreements = './results/bbox_label_agreement/label_agreements.csv'
    ...
    for mdl_master in model_classes:
        input_master = "./results/average_tranferability/{}/{}/{}/{}_transfer.csv".format(dataset, explainer, mdl_master, metrics_map[metric])
...
def main():
    ...
    #save direcory
    save_dir = "./results/analysis_transferability/{}".format(dataset)
    os.makedirs(save_dir, exist_ok=True)

    output_file='{}/transfer_{}_eps={}.csv'.format(save_dir, explainer, epsilon)
    df_all = pd.DataFrame()

    for metric in [1, 3, 4, 5]:
        df = compute_table(dataset, explainer, metric, epsilon)
