...
echo "checking results for rule lists"
                ...
                for model in "${models[@]}" 
                    do	
                        for metric in "${metrics[@]}"
                            do 
                                FILE="./results_rl/${dataset}/${model}/${metric}_${r}.csv"
...
echo "checking results for rule lists transfer"
for dataset in "${datasets[@]}" 
    do
        for r in ${rseed[@]}
            do
                for model in "${models[@]}" 
                    do	
                        for metric in "${metrics[@]}"
                            do 
                                FILE="./results_rl/${dataset}/${model}/${metric}_transfer_${r}.csv"
