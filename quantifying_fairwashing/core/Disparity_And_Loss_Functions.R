...
## ----- new loss function with kl penalty

.loss_logisticRegression_kl_helper <- function(y, u, kl, C_log = 5) {
  # implements the logistic regression loss given in Example 2 of 
  # Agarwal, Dudik and Wu (2019). The constant C_logistic is a pre-specificed
  # parameter that we always set to be equal to 5. 
  
  return( 
    log( ( 1 + exp(-C_log*(2*y-1)*(2*u-1)) ) ) / ( 2*log(1 + exp(C_log)) ) + kl
    )
}

compute_kl <- function (data, riskScore) {

  # confusion matrix for majority group
  cm_majority <- confusionMatrix(data=riskScore[data$A != protected_class], reference = data$Y[data$A != protected_class], positive="1")
  nTP_majority <- cm_majority$table[2,2]
  nFP_majority <- cm_majority$table[2,1]
  nTN_majority <- cm_majority$table[1,1]
  nFN_majority <- cm_majority$table[1,2]

  # confusion matrix for minority group
  cm_minority <- confusionMatrix(data=riskScore[data$A == protected_class], reference = data$Y[data$A == protected_class], positive="1")
  nTP_minority <- cm_minority$table[2,2]
  nFP_minority <- cm_minority$table[2,1]
  nTN_minority <- cm_minority$table[1,1]
  nFN_minority <- cm_minority$table[1,2]


  # normalized values for majority group
  TP_majority <- nTP_majority / (nTP_majority + nFP_majority)
  FP_majority <- nFP_majority / (nTP_majority + nFP_majority)
  TN_majority <- nTN_majority / (nTN_majority + nFN_majority)
  FN_majority <- nFN_majority / (nTN_majority + nFN_majority)

  # normalized values for minority group
  TP_minority <- nTP_minority / (nTP_minority + nFP_minority)
  FP_minority <- nFP_minority / (nTP_minority + nFP_minority)
  TN_minority <- nTN_minority / (nTN_minority + nFN_minority)
  FN_minority <- nFN_minority / (nTN_minority + nFN_minority)


  # kl divergence
  kl_TP <- TP_majority * log(TP_majority/TP_minority)
  kl_FP <- FP_majority * log(FP_majority/FP_minority)
  kl_TN <- TN_majority * log(TN_majority/TN_minority)
  kl_FN <- FN_majority * log(FN_majority/FN_minority)
  kl <- kl_TP + kl_FP + kl_TN + kl_FN
